...
import helium_enviar_correo as hel
...
argus.add_argument('mens', type=str,
                   help="Ubicacion del archivo con la plantilla del mensaje")

argus.add_argument('-l', '--listado',
                   default="Listado.ods",
                   type=str,
                   help="Nombre del archivo con las notas \
                           y correos y nombres.")

argus.add_argument('--hoysi', dest='to_all',
                   action='store_true',
                   default=False,
                   help="Enviar correo a todos los \
                   destinatario, sin esta opcion el \
                   correo solo se manda a Luis.")

argus.add_argument('--hist', dest='hist',
                   action='store_true',
                   default=False,
                   help="Generar y adjuntar un histograma \
                           de frecuencias de notas.")

argus.add_argument('-a', '--attach',
                   type=str,
                   help="Adjuntar un archivo al mensaje, \
                           por ejemplo el pdf de una tarea.")

argus.add_argument('-n', '--exanum',
                   type=str,
                   help="Especificar el encabezado de la \
                           columna a examinar ej. Exa 1")
    
argus.add_argument('-c', '--correos',
                   type=str, nargs='*',
                   help="Las columnas denominadas correos \
                           para sacar la lista de destinatarios")

argus.add_argument('-s', '--subject',
                   type=str,
                   help="El asunto del correo a enviar.")

args = argus.parse_args()

# CONVERTIMOS EL ARCHIVO A CSV
# unoconv_proc = subprocess.Popen(["unoconv", "-f", "csv",
# "-e", "FilterOptions=44,34,76",
# "-o", "/tmp/listado.csv", "%s"%"Listado.ods"])
# unoconv_proc.wait()

# aca pongo el examen que quiero
# es necesario porque de ahi saco los correos validos
if args.exanum is None:
    examen_num = None
else:
...
# debido a los correos mal
# ingreados al sistema)
if args.to_all:
    if args.correos is None:
...
def connect(email_address, email_port):
    try:
        ...
        serv.login(os.getenv("TEST_EMAIL"),
                   email_passw, initial_response_ok=True)
        print('logged in...')
    except socket.gaierror:
        raise RuntimeError('El correo de la U gaiaerror.')
    return serv


# serv = connect('outlook.office365.com', 587)
driver = hel.sign_in_w_2fa()
...
def histograma(grades, examen_num,  guardar_en='/tmp/Histogram.png'):
    ...
    mens_x = 26
    mens_y = 1.7
    plt.arrow(mens_x, mens_y, (-mens_x+nota)*0.95,
              -mens_y*0.8, head_width=0.01*mens_x,
              head_length=0.15*mens_y, color='red')
    plt.title('Percentil de tu nota: %s%% %s'
              % (L.pctl(nota, examen_num), examen_num))
...
for sublista in ListaDestinatarios:
    for li, cpo in zip(sublista, args.correos):
        if li:
            ...
            Diccion['pri_nombre'] =\
                L.buscar_por_campo(li, 'pn', campo=cpo).capitalize()
            ...
            msg['From'] = f'Luis Berlioz <{sender_email}>'
            msg['To'] = li
            msg['Date'] = formatdate(localtime=True)
            ...
            try:
                ...
                print('Al parecer %s no tiene %s' %
                      (Diccion['pri_nombre'], cpo))
                print('Ocurrio un KeyError: %s estas seguro que PAUTA esta \
                        bien escrito con mi correo en el campo de correo \
                        personal?' % K)
                sys.exit(1)
...
# ACA ADJUNTAMOS LA IMAGEN GENERADA CON MATPLOTLIB
            if args.hist:
                ...
                part.add_header('Content-Disposition', 'attachment; \
                        filename="{0}"'.format(
                            os.path.basename('/tmp/Histogram.png')))
                msg.attach(part)

            try:
                # serv.sendmail(os.getenv("TEST_EMAIL"), li, msg.as_string())
                hel.enviar(driver, li, args.subject,
                           MensajeCorreo)
                print('Email successfully sent to %s email: %s'
                      % (Diccion['pri_nombre'], li))
            except AttributeError:
                print('Fallo envio a %s con correo %s' %
                      (Diccion['pri_nombre'], li))
                ...
                # serv.sendmail('luis.berlioz@unah.edu.hn',
                #               li, msg.as_string())
                print('Email successfully sent to %s email: %s' %
                      (Diccion['pri_nombre'], li))
